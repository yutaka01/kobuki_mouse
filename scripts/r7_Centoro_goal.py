...
class turtlebot():
    ...
    def r6goal(self):
        print(mydict["r6"][0])
        goal_pose = Po()
        goal_pose.x = mydict["r6"][0]
        goal_pose.y = mydict["r6"][1]
        distance_tolerance = 0.1
        vel_msg = Twist()
        while True:
            _blockListDict = {
            ...
            #Creating our node,publisher and subscriber
            rospy.init_node('turtlebot_controller', anonymous=True)
            self.velocity_publisher = rospy.Publisher('r6/mobile_base/commands/velocity', Twist, queue_size=10)
            self.model_coordinates = rospy.ServiceProxy('/gazebo/get_model_state', GetModelState)
            pts = []
            name = []
            block = _blockListDict["r6"]
            blockName = str(block._name)
            self.resp_coordinates = self.model_coordinates(blockName, block._relative_entity_name)
            #pts = (resp_coordinates.pose.position.x, resp_coordinates.pose.position.y)
            #print(resp_coordinates)
    ...
    def r5goal(self):
        ...
        while True:
            ...
            #Creating our node,publisher and subscriber
            rospy.init_node('turtlebot_controller', anonymous=True)
            self.velocity_publisher = rospy.Publisher('r7/mobile_base/commands/velocity', Twist, queue_size=10)
            self.model_coordinates = rospy.ServiceProxy('/gazebo/get_model_state', GetModelState)
            pts = []
            name = []
            block = _blockListDict["r7"]
            blockName = str(block._name)
            self.resp_coordinates = self.model_coordinates(blockName, block._relative_entity_name)
            #pts = (resp_coordinates.pose.position.x, resp_coordinates.pose.position.y)
            #print(resp_coordinates)
...
if __name__ == '__main__':
    try:
        ...
        executor.submit(x.r6goal())
        executor.submit(x.r7goal())
